...
3. **No destructive tools.** No `rm`, `drop`, `truncate`, `kill -9`, `docker rm`, `DELETE`, no `apt-get remove`/`purge`, and no file writes outside `/var/log/bastion`. Not behind a flag, not behind a role. The one tool that changes the host's software, `install_package`, is install-only: a closed catalog of 12 keys mapped to fixed apt package lists in code, `admin` risk, approval-gated, run through `systemd-run` so the executor's own sandbox is never widened. `tests/test_tools_schema.py`, `tests/test_packages.py`, `tests/test_process_guard.py`, and the systemd unit (`ProtectSystem=strict`) enforce it.
4. **The executor validates everything.** LLM arguments are untrusted. Pydantic models with `extra="forbid"` validate types and bounds; extra validators check domains (`^[a-z0-9.-]+$` plus label rules), PID existence, and closed enums for service names and package catalog keys. `tests/test_executor_api.py`, `tests/test_executor_write_tools.py`, `tests/test_install_package_api.py`.
5. **Protected targets.** `kill_process` sends SIGTERM only and refuses pid 1, processes owned by `root`/`postgres`/`mysql`/`redis`, anything matching `postgres|mysqld|mariadbd|redis-server|sshd|systemd|dockerd|containerd|nginx: master|gunicorn: master`, the executor itself, and processes younger than 30 s. DB tools refuse `backend_type != 'client backend'` and replication users inside the SQL `WHERE` clause. `tests/test_process_guard.py`, `tests/test_executor_write_tools.py`.
...
- The executor trusts its host's `sudoers` file. `deploy/sudoers.bastion` lists literal commands only: `nginx -t`, one `systemctl restart <unit>` per service in the enum, two certbot forms (the domain regex-bound), and one `systemd-run … apt-get install -y <fixed list>` per package-catalog key plus `apt-get update -q`. The apt block is generated from the tool's own argv (`python -m bastion.tools --sudoers-apt`) and a test pins the committed file to it. `install.sh` validates the result with `visudo -c` before installing.
- `install_package` hands apt to PID 1 with `systemd-run --wait --pipe --collect`; the transient unit runs as root outside the executor's `ProtectSystem=strict` sandbox, exactly as an administrator running apt by hand would. The sandbox itself is unchanged. There is no rule and no tool for removing packages.
