...
| Executor process (`bastion` user) | trusted code, minimal privilege | run 21 fixed tools, a sudoers file of literal commands (incl. 13 generated apt lines), SIGTERM |
...
| 4 | **Destructive fix** proposed by the model | `kill -9`, `DROP TABLE`, `rm -rf /var/log`, `apt-get purge postgresql` | No such tools exist; tool-name denylist in the decorator; SIGTERM only; four fixed SQL statements; protected-process guard; sudoers lists literal commands only, none of which remove anything | `tools/process.py`, `tools/postgres.py`, `deploy/sudoers.bastion` |
...
| 13 | **Unwanted software on the host** | model installs `netcat`, pins a vulnerable version, adds a PPA, or removes postgresql | `install_package` accepts one closed-enum catalog key (12 entries) mapped to fixed apt lists in code; `admin` risk (hidden unless `enabled_risks` lists `admin` *and* the token is admin); plan shows both exact `apt-get` lines and `/run` needs their hash; each sudoers line is literal (no wildcards) and generated from the tool's argv; no remove/purge tool or rule; apt runs in a `systemd-run` transient unit so the executor sandbox is not widened; adversarial suite asserts every `systemd-run` argv reaching sudo is a catalog argv | `tools/packages.py`, `tools/_types.py`, `deploy/sudoers.bastion`, `tests/test_install_flow.py`, `tests/adversarial/` |
...
- `install_package` trusts the distribution's apt repositories and whatever version they ship. Bastion does not verify package signatures beyond what apt does, and a compromised mirror is out of scope. apt output (up to 40 tail lines per command) reaches the model redacted and fenced like any other tool output.
- Installing a database or web server starts it with the distribution's default configuration. Bastion cannot configure it (no config-editing tool); the operator does that by hand.
